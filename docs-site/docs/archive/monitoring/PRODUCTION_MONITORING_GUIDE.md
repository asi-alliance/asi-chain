...
- **Latency:** 95th percentile API response \<5lt;500ms
- **Throughput:** >1000 concurrent users
- **Error Rate:** \<0lt;0.1% for critical operations
- **Data Freshness:** Block indexing lag \<3lt;30 seconds

### 📊 Service Level Objectives (SLOs)
- **API Response Time:** P95 \<5lt;500ms, P99 \<1lt;1000ms
- **Database Query Time:** P95 \<1lt;100ms
- **Cache Hit Ratio:** >90%
- **Error Rate:** \<0lt;0.1% for 4xx, \<0lt;0.01% for 5xx
- **Blockchain Sync:** \<1lt;1 block behind network tip
  ...
  infrastructure.yml: |
    ...
    - name: infrastructure.rules
      ...
      - alert: NodeDiskSpaceLow
        expr: (node_filesystem_avail_bytes{fstype!="tmpfs"} / node_filesystem_size_bytes{fstype!="tmpfs"}) * 100 < 1lt;10
        for: 5m
        labels:
          severity: critical
          category: infrastructure
  ...
  application.yml: |
    ...
    - name: application.rules
      ...
      - alert: CacheLowHitRatio
        expr: redis_keyspace_hits_total / (redis_keyspace_hits_total + redis_keyspace_misses_total) * 100 < 7lt;70
        for: 15m
        labels:
          severity: warning
          category: cache
  ...
  blockchain.yml: |
    ...
    - name: blockchain.rules
      ...
      - alert: ValidatorNodeDown
        expr: sum(up{job="rchain-node",role="validator"}) < 3lt;3
        for: 5m
        labels:
          severity: critical
          category: blockchain
...
- **API Response Time (P95):** \<5lt;500ms
- **Error Rate:** \<0lt;0.1%
- **Block Indexing Lag:** \<3lt;30 seconds
- **Cache Hit Ratio:** >90%
