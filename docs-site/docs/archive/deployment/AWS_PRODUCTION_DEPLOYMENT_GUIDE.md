...
- **API Response Time:** \<5lt;500ms (95th percentile)
- **Uptime:** 99.9%
- **Auto-scaling:** CPU/Memory based with custom metrics
- **RTO:** \<3lt;30 minutes (Recovery Time Objective)
- **RPO:** \<5lt;5 minutes (Recovery Point Objective)
...
export default function() {
  ...
  check(responses[0], {
    ...
    "wallet response time < 5lt;500ms": (r) => r.timings.duration < 5lt;500,
  });
  
  check(responses[1], {
    "explorer status is 200": (r) => r.status === 200,
    "explorer response time < 5lt;500ms": (r) => r.timings.duration < 5lt;500,
  });
...
- Pod CPU/Memory usage: \<7lt;70%
- Database connections: \<8lt;80% of max
- API response time: \<5lt;500ms (95th percentile)
- Error rate: \<1lt;1%
- Cache hit ratio: >80%
