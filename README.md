...
const backupCodes = await wallet.generate2FABackupCodes();
...
if (isAvailable) {
  ...
  const credential = await wallet.registerBiometric({
...
const authenticated = await wallet.authenticateWithBiometric();
```
...
const trezor = await wallet.connectTrezor();

// Get addresses from Trezor
const addresses = await trezor.getAddresses(0, 5); // First 5 addresses

// Sign with Trezor
const signature = await trezor.signTransaction(transaction);
```
...
# - Ledger Nano S/X
# - Trezor Model T
# - Keep offline backups of recovery phrases
...
# Use physical, offline storage only
```
...
**Problem: Hardware wallet not detected**
```bash
# Symptoms:
"No hardware wallet found" in browser

# Solutions:
# 1. Enable WebUSB in browser
# Chrome: chrome://flags/#enable-web-usb-api

# 2. Install Ledger app
# - Connect Ledger device
# - Install Ethereum app
# - Enable "Contract data" in app settings
...
export SBT_OPTS="-Xmx2g -Xss1m -XX:+UseG1GC"
```
...
**JVM Tuning:**
```bash
# Optimal JVM settings for F1R3FLY
export SBT_OPTS="-Xmx4g -Xss2m -XX:+UseG1GC -XX:+UseStringDeduplication"
