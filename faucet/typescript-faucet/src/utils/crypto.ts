...
import { Deploy, SignedDeploy, KeyPair } from '../types';
...
export const validateRevAddress = (address: string): boolean => {
  try {
    if (!address || !address.startsWith('1111')) {
      return false;
    }
    
    // Decode and check length  
    const decoded = bs58.decode(address);
    // Valid REV addresses should decode to 40-43 bytes
    return decoded.length >= 40 && decoded.length <= 43;
  } catch {
    return false;
  }
};
...
const deployDataProtobufSerialize = (deployData: Deploy): Uint8Array => {
...
export const signDeploy = (deployData: Deploy, privateKey: string): SignedDeploy => {
  const keyPair = secp256k1.keyFromPrivate(privateKey, 'hex');
