...
def send_deploy(deploy):
    """Send the signed deploy to the validator node"""
    try:
...
def get_balance(rev_address):
    """Get REV balance for an address using Rholang explore-deploy"""
    try:
        # Use the same Rholang code as the wallet
        check_balance_rho = f'''
        ...
        # Send explore-deploy to read-only node
        response = requests.post(
        ...
        if response.status_code == 200:
            ...
            if result and 'expr' in result and len(result['expr']) > 0:
                ...
                # Check if it's a direct integer (balance in dust)
                if 'ExprInt' in first_expr and 'data' in first_expr['ExprInt']:
                    return first_expr['ExprInt']['data']
                # Check if it's an error string
                if 'ExprString' in first_expr and 'data' in first_expr['ExprString']:
                    print(f"Balance check error: {first_expr['ExprString']['data']}")
                    return 0
...
def request_tokens():
    ...
    try:
        ...
        if not address or not address.startswith('1'):
            return jsonify({'success': False, 'error': 'Invalid REV address format'}), 400
        ...
        # Create and send the transfer deploy
        deploy = create_transfer_deploy(
            faucet_rev_address,
            address,
            FAUCET_AMOUNT,
            FAUCET_PRIVATE_KEY
